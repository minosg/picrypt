...
int main(int argc, const char *argv[])
{
  ...
  sh_encrypt_string(test_string, cr_string, sizeof(cr_string));
  sh_encrypt_string(ref_string, rf_string, sizeof(rf_string));

  /* Decrypt the user string */
  sh_decrypt_string(cr_string, dec_string, sizeof(cr_string), sizeof(dec_string));
  printf("Encrypting %s stored as:\n", test_string);
  sh_print_array(cr_string,sizeof(cr_string));
  printf("Decrypted as: \"%s\"\n", dec_string);

  if (sh_compare_encrypted_str(rf_string,
                               cr_string,
                               sizeof(rf_string),
                               sizeof(cr_string)))
  {
